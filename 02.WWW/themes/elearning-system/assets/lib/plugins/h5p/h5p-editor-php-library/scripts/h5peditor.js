    ...
    if (libraryData.javascript !== undefined) {
      ...
      for (var path in libraryData.javascript) {
        ...
        //  H5PIntegration.loadedJs.push(path);
            ns.$('head').append("<script type='text/javascript' src='"+path+"'></script>");
...
ns.loadLibrary = function (libraryName, callback) {
  switch (ns.libraryCache[libraryName]) {
    ...
    case undefined:
     ...
     url = H5PIntegration.baseUrl + "lib/plugins/h5p/json/"+ library.machineName +".json";
        // console.log(url);
      // Fire away!
      ns.$.getJSON( url, function( libraryData ) {
